...
import {{project.projectPackage}}.config.liquibase.AsyncSpringLiquibase;
...
import {{project.projectPackage}}.config.oauth2.OAuth2AuthenticationReadConverter;
<%_ } _%>
<%_ if (databaseType == 'mongodb') { _%>

import {{project.projectPackage}}.domain.util.JSR310DateConverters.*;
...
@EnableJpaRepositories("{{project.projectPackage}}.repository")
@EnableJpaAuditing(auditorAwareRef = "springSecurityAuditorAware")
@EnableTransactionManagement<% } %><% if (searchEngine == 'elasticsearch') { %>
@EnableElasticsearchRepositories("{{project.projectPackage}}.repository.search")<% } %><% if (databaseType == 'mongodb') { %>
@Profile("!" + Constants.SPRING_PROFILE_CLOUD)
@EnableMongoRepositories("{{project.projectPackage}}.repository")
...
<%_ } _%>
    ...
    public Mongobee mongobee(Mongo mongo, MongoProperties mongoProperties) {
        ...
        mongobee.setChangeLogsScanPackage("{{project.projectPackage}}.config.dbmigrations");
